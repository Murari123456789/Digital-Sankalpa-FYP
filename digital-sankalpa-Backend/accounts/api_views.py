...
def update_profile(request):
    try:
        ...
        if serializer.is_valid():
            user = serializer.save()
            # Return the updated user data along with success message
            return Response({
...
def change_password(request):
    try:
        ...
        if not old_password or not new_password:
            return Response({"error": "Both old password and new password are required"}, 
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            # Return validation errors from Django's password validator
